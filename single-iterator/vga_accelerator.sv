...
module vga_polygon_accelerator #(
    ...
    localparam logic [31:0] FP_16384 = 32'h4680_0000;
    // Match C: samples every Nth column (see N_PIXEL_SSE in C)
    localparam logic [31:0] SSE_STRIDE = 32'd10;
    ...
    logic [2:0] r1, g1, r2, g2;
    logic [1:0] b1, b2;
    logic [15:0] scaled_r1, scaled_g1, scaled_b1, scaled_r2, scaled_g2, scaled_b2;

    logic signed [17:0] diff_r, diff_g, diff_b;
    logic [35:0]        diff_sq_r, diff_sq_g, diff_sq_b;
    logic [31:0]        current_pixel_sse;
    logic [63:0]        sse_accum;
    logic [31:0]        pixel_count;
    ...
    function automatic logic [31:0] clamp_x_to_window(input logic signed [31:0] x_in);
        begin
            if (x_in < X_MIN)
                clamp_x_to_window = 32'd0;
            else if (x_in > X_MAX)
                ...
                clamp_x_to_window = unsigned'(x_in);
        end
    endfunction

    // RGB332 -> ~0..255 (same idea as host: r,g 3-bit, b 2-bit)
    assign {r1, g1, b1} = color;
    assign {r2, g2, b2} = m10k_rd_data;
    assign scaled_r1 = (unsigned'(r1) * 16'd255 + 16'd3) / 16'd7;
    assign scaled_g1 = (unsigned'(g1) * 16'd255 + 16'd3) / 16'd7;
    assign scaled_b1 = (unsigned'(b1) * 16'd255 + 16'd1) / 16'd3;
    assign scaled_r2 = (unsigned'(r2) * 16'd255 + 16'd3) / 16'd7;
    assign scaled_g2 = (unsigned'(g2) * 16'd255 + 16'd3) / 16'd7;
    assign scaled_b2 = (unsigned'(b2) * 16'd255 + 16'd1) / 16'd3;

    assign diff_r = $signed({1'b0, scaled_r1}) - $signed({1'b0, scaled_r2});
    assign diff_g = $signed({1'b0, scaled_g1}) - $signed({1'b0, scaled_g2});
    assign diff_b = $signed({1'b0, scaled_b1}) - $signed({1'b0, scaled_b2});

    assign diff_sq_r = diff_r * diff_r;
    assign diff_sq_g = diff_g * diff_g;
    assign diff_sq_b = diff_b * diff_b;
    assign current_pixel_sse = diff_sq_r[31:0] + diff_sq_g[31:0] + diff_sq_b[31:0];
    ...
    always_comb begin
        int e;
        row_num_hits = 3'd0;
        for (e = 0; e < 4; e++) row_hits_unsorted[e] = 32'sd0;
    ...
    always_comb begin
        int i, j;
        logic signed [31:0] t;
        for (i = 0; i < 4; i++) row_hits_sorted[i] = row_hits_unsorted[i];
        for (i = 0; i < 3; i++)
            for (j = i + 1; j < 4; j++)
                if ((j < row_num_hits) && (row_hits_sorted[j] < row_hits_sorted[i])) begin
                    t = row_hits_sorted[i];
            ...
            case (state)
                ...
                FIND_BOUNDS: begin
                    if (n_vertices < 3) begin
                        ...
                        for (i = 1; i < 4; i++)
                            if (i < n_vertices) begin
                                if (py[i] < min_y_local) min_y_local = py[i];
                                if (py[i] > max_y_local) max_y_local = py[i];
                            end
                        // Match C: clamp row range to the framebuffer
                        if (min_y_local < 32'sd0)
                            min_y_local = 32'sd0;
                        if (max_y_local > signed'(WINDOW_HEIGHT - 1))
                            max_y_local = signed'(WINDOW_HEIGHT - 1);
                ...
                CALC_EDGES: begin
                    ...
                    for (i = 0; i < 4; i++) begin
                        if (i < n_vertices) begin
                            ...
                            if (y_a < y_b) begin
                                y_min_local = y_a; y_max_local = y_b;
                                x_start_local = x_a;
                                dx_local = x_b - x_a;
                                dy_local = y_b - y_a;
                            end else begin
                                y_min_local = y_b; y_max_local = y_a;
                                x_start_local = x_b;
                                dx_local = x_a - x_b;
                                dy_local = y_a - y_b;
                            end

                            if (dy_local != 32'd0) begin
                                edge_valid[i] <= 1'b1;
                                edge_y_min[i] <= y_min_local;
                                edge_y_max[i] <= y_max_local;
                                edge_x[i] <= x_start_local;
                ...
                SORT_HITS: begin
                    if (row_num_hits < 2) begin
                        ...
                        row_pixels_expected <=
                            (clamp_x_to_window(row_hits_sorted[1]) - clamp_x_to_window(row_hits_sorted[0]))
                            / SSE_STRIDE + 32'd1;
                ...
                CALC_SCORE_WAIT_1: state <= CALC_SCORE_WAIT_2;
                CALC_SCORE_WAIT_2: state <= CALC_SCORE_WAIT_3;
